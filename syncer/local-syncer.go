...
func (syncer *SftpLocalSyncer) uploadable(file_to_download string, output_file string, stat fs.FileInfo) bool {
	...
	// syncer.logger.Debugf(fmt.Sprintf("uploadable: found %s", output_file))
	remote_size := remote_stat.Size()
	local_size := stat.Size()
	if local_size != remote_size {
		// syncer.logger.Debugf(fmt.Sprintf("uploadable: %s size different %d %d", output_file, remote_size, local_size))
		return true
	}
	// syncer.logger.Debugf(fmt.Sprintf("uploadable: %s size same %d %d", output_file, remote_size, local_size))
	remote_modtime := remote_stat.ModTime().Unix()
	local_modtime := stat.ModTime().Unix()
	// syncer.logger.Debugf(fmt.Sprintf("uploadable: %s time %d %d", output_file, remote_modtime, local_modtime))
	return local_modtime != remote_modtime
}

func (syncer *SftpLocalSyncer) upload(file_to_upload string, output_file string) {
	syncer.logger.Debugf(fmt.Sprintf("uploading file %s to %s:%s", file_to_upload, syncer.Server, output_file))
	output_parent_folder := strings.ReplaceAll(filepath.Dir(output_file), "\\", "/")
	err := syncer.sftp_client.MkdirAll(output_parent_folder)
	if err != nil {
		syncer.logger.Errorf(fmt.Sprintf("unable to create remote folder: %s: %s: %s", syncer.Server, output_parent_folder, err.Error()))
		syncer.to_exit = true
		return
	}
	...
	if err != nil {
		syncer.logger.Errorf(fmt.Sprintf("unable to open local file: %s: %v", file_to_upload, err))
		return
	}
	defer source.Close()
	...
	if openerr != nil {
		syncer.logger.Errorf(fmt.Sprintf("error opening remote file: %s:%s: %v", syncer.Server, output_file, err))
		syncer.to_exit = true
		return
	}
	defer target.Close()
	...
	if err != nil {
		syncer.logger.Errorf(fmt.Sprintf("error uploading file: %s: %v", file_to_upload, err))
		syncer.to_exit = true
		return
	}
	...
	syncer.logger.Infof(fmt.Sprintf("uploaded %s with %d bytes in %d ms, %.1f mbps", file_to_upload, nBytes, time_taken, float64(nBytes/1000*8/time_taken)))
}
...
func (syncer *SftpLocalSyncer) init(loggerInstance *logger.Logger) {
...
func (syncer *SftpLocalSyncer) Start(c chan uploader.FileObj, done chan int, loggerInstance *logger.Logger) {
